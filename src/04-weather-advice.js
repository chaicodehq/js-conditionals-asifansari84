...
export function getWeatherAdvice(temperature, isRaining) {
  if(temperature >= 35) {
    temperature = "Too hot for hiking - stay indoors and hydrate";
    } 
    else if(temperature >=25 && !isRaining) {
      temperature =  "Great weather for hiking - don't forget sunscreen";
    }
    else if(temperature >= 25 && isRaining) {
      temperature =  "Warm but rainy - consider indoor activities";
    }
    else if(temperature >= 15 && !isRaining) {
      temperature =  "Perfect hiking weather - enjoy the trails";
    }
    else if(temperature >= 15 && isRaining) {
      temperature =  "Cool and rainy - bring waterproof gear if hiking";
    }
    else if(temperature >= 5 && !isRaining) {
      temperature =  "Chilly - wear layers for your hike";
    }
    else if(temperature >= 5 && isRaining) {
      temperature =  "Cold and wet - best to stay indoors";
    }
    else if(temperature < 5) {
      temperature =  "Too cold - stay warm indoors";
    }
  return temperature
}
